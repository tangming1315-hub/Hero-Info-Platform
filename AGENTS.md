...
> **写给 AI 看的约束，也是写给人的协作规则。** 修改本文件必须走 PR 流程，与代码同等对待。
...
- **仓库**：https://github.com/tangming1315-hub/Hero-Info-Platform
- **目录结构**（TODO：技术栈确定后补充）
  ```
  Hero-Info-Platform/
...
**铁律**：提交 PR 前必须本地通过 Lint 和测试，AI 代理完成编码任务后必须自行执行上述命令验证。
...
### PR 规范
- **所有代码（含 AI 生成）必须走 PR，禁止直推 main**
- PR 必须按模板填写「AI 参与说明」：哪些部分由 AI 生成、人工做了哪些验证
- PR 通过自动化检查（lint + test）后才能合入
- 任务拆分原则：大需求拆成边界清晰、输入输出明确的子任务，一个 PR 对应一个子任务

### 并行协作冲突规避
- 开工前先 `git pull` 同步主干
- 跨模块改动先在群里/PR 中声明涉及文件，避免与他人任务撞车
...
3. **产出即验证**：AI 交付前必须运行构建/测试/Lint，验证结果写进 PR 描述
4. **失败要留痕**：AI 踩过的坑、验证过的结论，沉淀到本文件或 `docs/` 下，供全团队复用
5. **本文件是活文档**：规范演进 = 修改本文件 + 提 PR + Review，禁止只在群里口头约定

## 9. 文档与知识沉淀

- 架构决策记录（ADR）：重要取舍放 `docs/adr/`，记录背景、选项、结论，不得静默覆盖历史决策
- 变更记录：功能级变更随 PR 更新对应文档
- TODO：确定后补充文档目录结构
